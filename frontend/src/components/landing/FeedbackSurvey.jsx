...
const QUESTIONS = [
  {
    ...
    placeholder:
      "E.g. Performance analyst at a Ligue 2 club / Coach at an academy / Scout / Solo content creator…",
    type: "input",
  },
  {
    key: "workflow",
    ...
    placeholder:
      "How does a match-week look for you? Tools you use, where you spend time…",
    type: "textarea",
  },
  {
    key: "time_sink",
    ...
    placeholder:
      "Tagging events, syncing video, building reports, sharing with staff…",
    type: "textarea",
  },
  {
    key: "pain_point",
  ...
  },
  {
    ...
    placeholder:
      "Auto reports, plain-English Q&A, Telegram bot, physical stats, event detection…",
    type: "textarea",
  },
  {
    key: "workflow_change",
...
const EMPTY_VALUES = QUESTIONS.reduce(
  (acc, q) => ({ ...acc, [q.key]: "" }),
  { email: "" }
);
  ...
  return (
    <div className="lg:col-span-5">
      ...
      <p className="mt-4 text-base leading-relaxed text-white/65">
        Six short, optional questions. Skip anything you want — even one answer
        helps PREDA ship the right features for clubs and academies.
      </p>
...
function StepHeader({ step, total, progress }) {
  return (
    <>
...
function QuestionField({ question, value, onChange }) {
  const sharedClasses =
    "rounded-xl border-white/10 bg-white/5 text-white placeholder:text-white/35 focus-visible:ring-2 focus-visible:ring-[hsl(var(--ring))]";
  return (
    <motion.div
      key={question.key}
      ...
      <div className="mt-4">
        {question.type === "input" ? (
          ...
          <Textarea
            ...
            value={value}
            onChange={(e) => onChange(question.key, e.target.value)}
            className={`min-h-[120px] ${sharedClasses}`}
          />
        )}
      </div>
    </motion.div>
...
function SurveyForm({
  ...
  return (
    <div className="p-5 sm:p-7">
      <StepHeader step={step} total={QUESTIONS.length} progress={progress} />
      ...
      {isLast && <EmailField value={values.email} onChange={onChange} />}
      <StepControls
        step={step}
        isLast={isLast}
...
function SurveySuccess({ onReset }) {
  return (
    <motion.div
    ...
    >
      ...
      <p className="max-w-md text-sm text-white/65">
        We read every submission. If you left an email, we’ll reach out as soon
        as PREDA opens its first invite cohort.
      </p>
      <Button
        variant="ghost"
        onClick={onReset}
...
export default function FeedbackSurvey() {
  ...
  const submit = async () => {
    if (!hasAnyAnswer(values)) {
      toast.error(
        "Add at least one answer or your email so we can follow up."
      );
      return;
    }
